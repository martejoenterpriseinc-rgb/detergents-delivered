...
export async function GET(_request: Request, { params }: { params: Promise<{ key: string[] }> }) {
  const gate = await requireApiRole(["ADMIN", "INVENTORY", "CPA", "SUPER_ADMIN", "DRIVER"]);
  ...
  try {
    const bytes = await readLocalObject(storageKey.startsWith("local/") ? storageKey : `local/${storageKey}`);
