...
| Measured route mileage and audited corrections                       | 70    | 161     | 696   |
| Manual-payment approvals and separate delivery tips                  | 71–72 | 168     | 716   |
| Immediate refund accounting warnings and recovery preservation       | 73    | 170     | 716   |
...
PR 68 adds the read-only CPA financial event report and private CSV at source `a6ff05f353b4808c5f6d7088fb822211be718c2b`, tree `8cce9faf3d89b2e722f04545647bf00d40cc0513`. This is a prior fully tested application candidate. CI 156, run `34663809041`, job `103471563797`, passed 366 unit, 250 native PostgreSQL and 71 browser checks (687 total), with lint, typecheck, schema validation, migration replay and production build. Evidence artifact: `10288585609`. Final-revision CPA screenshots were inspected at desktop, tablet and mobile widths, along with the corrected desktop receipt capture. The report verifies original sale/refund/reward/tax/FIFO source records, keeps missing evidence unknown, separates sellable/damaged return costs from refunds, and bounds complete exports at 250 events. See `CPA-FINANCIAL-LEDGER.md`. Initial build typed-link and browser warning-locator failures were corrected before this successful run. It adds no migration or provider acceptance.

PR 70 adds precise route mileage at source `54d5b5d536a72cb16ab7529004c1d9429bb5a3fc`, tree `30061b6f54dcd21b36de5acfb6470706a5173135`. CI 161, run `34666094409`, job `103478157172`, passed 369 unit, 253 native PostgreSQL and 74 browser checks (696 total), plus all build gates. Desktop/tablet/mobile mileage screens were reviewed from downstream CI 165 artifact `10289583178`; those mileage tests also passed. This adds the route-mileage migration below, with no edits to retained migrations.

PRs 71–72 add manual-payment approval controls and separate delivery tips at source `d626b4831f416d51a3021ca37268df38d39509a3`, tree `e6c16529b5c1bbcbca173c7e9b913830d8d5f58d`. CI 168, run `34692154047`, job `103549198496`, passed 376 unit, 260 native PostgreSQL and 80 browser checks (716 total), plus all build gates. The approval flow's lost-save/retry/revocation/history cases pass at desktop/tablet/mobile widths after explicit dropdown accessible names were added. Approval screenshots were inspected from focused artifact `10297426689`; unchanged tip/mileage screens were inspected from CI 165 artifact `10289583178`. Prior CI 164/165 label failures were corrected. Standalone approval CI 167 first attempt timed out in the older desktop delivery polling case, which passes in combined CI 168; that failed job is rerunning. These are application results, not provider acceptance.

PR 73 adds immediate refund accounting-correction warnings at source `7d2ac7b73e8a191c6af00e23463e488fadc6db5c`, tree `540da214e3d025df46febebdc1fc8c2165e47fab`. This is the latest fully tested application candidate. CI 170, run `34692354890`, job `103549745464`, passed 376 unit, 260 native PostgreSQL and 80 browser checks (716 total), plus lint, typecheck, schema validation, migration replay and production build. Focused screenshot artifact: `10297696570`. The unchanged application screens have the visual review described above. CI 169 caught a recovery-timestamp regression after uncertain receipt submission; the fix preserves an immediately due first worker read without erasing correction warnings. Native coverage verifies immediate correction warnings, duplicate replay, full compensation rollback when the warning audit fails, preservation of original receipts and warning retention through provider outages. Existing recovery coverage proves one provider submission and later read-only worker confirmation. No additional migration or provider correction transaction is introduced.
...
The last recorded hosted revision is `a2495c4afa459a9c258804841fa37b038ecdd828`. The current build adds the sixteen migrations below; no previously retained migration SQL is modified. These add accounting/consent/cycle evidence, scheduling metadata, measured route mileage, manual-payment approvals and separate delivery tips, and extend refund constraints to support reward-only returns. Legacy subscription rows are preserved and are not silently converted into consented quarterly subscriptions.
...
| `20260916230000_route_mileage_records`              | `cee24880233ff9f8b78807a9eb720df307eea0cddaf192507984b43f978032ca` |
| `20260916233000_manual_payment_approvals`           | `1d39697f97d581fcb5590cb60466250f4cee81d5bf1e48fc8c90d0fa2aee2d7a` |
| `20260917000000_delivery_tips`                      | `84a0a1efc408e1bf6f0f65c570c180d4f251a6a7724c525a9832506e04119017` |
...
The current complete gap list is [REMAINING-GO-LIVE-BUILD.md](REMAINING-GO-LIVE-BUILD.md), including manual-payment settlement integration, tip refund/payout accounting, provider correction evidence and final workflow review. Do not describe external API connections as the only unfinished work.
