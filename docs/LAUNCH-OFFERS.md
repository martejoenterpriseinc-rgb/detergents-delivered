...
This extends the retained Render-linked app after local commits `21e9c8a` and `4bc34ec`. The continuation published this retained source and deployed staging after native/browser acceptance; no rebuild, provider calls, domain change, production write or customer message occurred. The previously saved audit and approved interface history remain intact. The original attachment files are not present in this restored workspace; the saved audit/architecture/testing documents and the user's full conversation requirements informed this increment.

## Requirement matrix

| Requirement | Implementation | Accepted proof / remaining limit |
| --- | --- | --- |
| Admin Shop online / Add inventory | `/admin/shop` and `/admin/shop/new`; title, description, SKU, selling/retail prices, computed savings, optional image, publish flag and vehicle load type/units | Product and prices are one atomic transaction with audit and idempotent replay. Quantities still come from Receiving. Native creation/idempotency and desktop/mobile creation/refresh passed in CI47 |
...
| Support KPI | Existing overview Support tickets KPI links to `/admin/support?status=ACTIVE` | Support workflow and new KPI click-through pass in the CI47 regression run |
| Launch date / order cutoff | `/admin/settings/launch` saves explicit launch date, inclusive Chicago cutoff date and first-delivery deadline. October 15–31 draft saved on staging; public window remains disabled | Enabling the banner does not open paid orders. Contact/support captures enquiries; no dedicated interest-order pipeline or preorder charge path is claimed |
...
`20260909210000_launch_offers` is additive: nullable account approval/address validation, product load kind, safe-zero vehicle limits and promotion terms. Old migrations are unchanged. No customer/order/payment/receipt/reward record is backfilled or deleted. Both new migrations were applied to staging after backup and native replay acceptance; existing table columns/rows were verified unchanged. Existing source permits only synthetic native-test fixtures to populate approval/validation fields; no public setter accepts them.
...
## Accepted evidence and remaining blocks

CI47 at `b986ee320d857cc47e6656b3007bb079d02980cf` passes 138 unit checks, 38 native PostgreSQL integrations, all 10 desktop/mobile browser cases, lint/typecheck/build and migration replay. This includes all four new launch-offers database cases and both launch-offers browser projects. Successful sessions/database saves are real synthetic local CI operations; HTTP 503 injection and local storage adapters are explicitly distinguished from provider sandbox calls.

Staging deployment, unchanged-record assertions, saved date/revision/audit results, exact run/artifact links and the current acceptance matrix are in [LAUNCH-DATE-ACCEPTANCE.md](LAUNCH-DATE-ACCEPTANCE.md). Historical publication/non-root local runtime blockers were resolved through owner-approved connected GitHub publication and the isolated native PostgreSQL CI runner; no database guard was bypassed.

The form shows the last confirmed saved launch date and flags unsaved changes. Changed-date/revision persistence, refresh, unauthorized/invalid writes, concurrent writes and failed-save recovery pass. Hosted owner save and a second browser tab confirm the stored date.

**Launch verdict: NO PRODUCTION LAUNCH.** Provider checkout/tax, atomic stock/capacity finalization, verified customer/address onboarding, durable private storage, reliable workers and receipt/refund/reconciliation acceptance remain gaps.
