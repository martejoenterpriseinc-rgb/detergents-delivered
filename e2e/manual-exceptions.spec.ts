...
test("provider non-creation review retains evidence and the exact request after a lost response", async ({
  page,
}, info) => {
  const db = new PrismaClient({ log: [] }),
    password = "Synthetic-Claim-Review-123",
    f = await manualPaymentFixture(db, await bcrypt.hash(password, 4));
  const marker = randomUUID();
  ...
  const receipt = await db.manualCheckoutSettlement.create({
    data: {
      id: "claim_" + marker,
      checkoutId: checkout.id,
      method: "ZELLE",
      reference: "receipt-" + marker,
      amountCents: 1080,
      receivedAt: new Date(),
      actorUserId: f.admin.id,
      reason: "Synthetic received money",
      approvalId: "synthetic",
      approvalVersion: 1,
      requestHash: "synthetic",
      accountId: "acct_synthetic",
      livemode: false,
      state: "UNKNOWN",
      submittedAt: new Date(Date.now() - 49 * 3600000),
    },
  });
  ...
  try {
    expect(
      (
        await page.request.post("/api/admin/financial-claim-reviews", { data: {} })
      ).status(),
    ).toBe(401);
    await page.goto("/sign-in");
    await page.getByLabel("Email", { exact: true }).fill(f.admin.email!);
    ...
    const row = page
      .getByRole("article")
      .filter({ has: page.getByRole("heading", { name: order.number, exact: true }) });
    await row.getByRole("button", { name: "Load claim reviews", exact: true }).click();
    await row
      .getByLabel("Provider support case or request-log reference")
      .fill("case-synthetic-review");
    await row
      .getByLabel("Retained evidence location or document reference")
      .fill("retained/provider-confirmation.pdf");
    await row
      .getByLabel("Retained provider confirmation file", { exact: true })
      .setInputFiles({
        name: "synthetic-provider-confirmation.txt",
        mimeType: "text/plain",
        buffer: Buffer.from("Synthetic provider confirmation: no transaction created."),
      });
    await row
      .getByLabel("Provider confirmation covers through (your local time)")
      .fill("2026-09-13T12:00");
    await row
      .getByLabel("Evidence summary")
      .fill(
        "Synthetic provider confirmation for the original account and request; all attempts have ended.",
      );
    await row
      .getByRole("checkbox", { name: /The provider confirms this exact request/ })
      .check();
    await page.route("**/api/admin/financial-claim-reviews", async (route) => {
      requests.push(route.request().postDataJSON());
      if (requests.length === 1) return route.abort("failed");
      return route.fulfill({ json: { id: "synthetic-review", decision: "PENDING" } });
    });
    await row
      .getByRole("button", { name: "Propose non-creation review", exact: true })
      .click();
    await expect(
      row.getByRole("button", { name: "Retry same review action", exact: true }),
    ).toBeVisible();
    await expect(row.getByLabel("Evidence summary")).toBeDisabled();
    await row
      .getByRole("button", { name: "Retry same review action", exact: true })
      .click();
    await expect(row.getByRole("status")).toContainText("Review PENDING");
    expect(requests).toHaveLength(2);
    expect(requests[1]).toEqual(requests[0]);
    expect(requests[0]).toMatchObject({
    ...
    expect(requests[0].evidenceSha256).toMatch(/^[a-f0-9]{64}$/);
    expect(
      await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth),
    ).toBe(true);
    await page.screenshot({ path: info.outputPath("claim-review.png"), fullPage: true });
  } finally {
    await f.cleanup();
    await db.$disconnect();
  }
