...
import { effectiveRefundCents } from "@/lib/domain/refund-settlement";
...
export async function reviewReferral(userId: string, referralId: string) {
  return prisma.$transaction((tx) => reviewReferralInTransaction(tx, userId, referralId));
}
...
export async function reviewReferralInTransaction(
    ...
    .update(
      JSON.stringify(
        order.refunds
          .map((r) => ({
            id: r.id,
            adjustments: r.request?.adjustments.map((a) => a.id).sort(),
          }))
  ...
  if (referral.status === "REVERSED") {
    const compensated = order.refunds.some((r) =>
      r.request?.adjustments.some((a) => a.kind === "COMPENSATION"),
    );
    if (
      refunded ||
      !compensated ||
      !referral.redeemedAt ||
      (await capturedPayment(tx, order.id)) !== order.totalCents
    )
    ...
    for (const [customerId, amountCents, suffix] of [
      ...
      const net = await tx.rewardEntry.aggregate({
        where: { sourceId: referral.id, customerId },
        _sum: { amountCents: true },
      });
      if ((net._sum.amountCents ?? 0) !== 0)
        throw new AccountError("Referral reward history requires reconciliation.", 409);
      if (amountCents)
        await entry(
          tx,
          {
            ...
            amountCents,
            kind: "RESTORE",
            entryKey: `referral:${referral.id}:restore:${suffix}:${revision}`,
      ...
      if (amountCents)
        await entry(
          tx,
          {
            ...
            kind: "REVERSAL",
            entryKey: `referral:${referral.id}:reverse:${suffix}:${revision}`,
            sourceId: referral.id,
            orderId: order.id,
